...
const MatchingFeaturesReading = () => {
  const [open, setOpen] = useState(false);
  const [select, setSelect] = useState("Choose the correct option");

  const options = ["Features 1", "Features 2", "Features 3", "Features 4"];

  useEffect(() => {
    const saved = localStorage.getItem("selectFeatures");
    if (saved) {
      setSelect(saved);
    }
  }, []);
  ...
  const handleSelect = (opt: string) => {
    setSelect(opt);
    localStorage.setItem("selectFeatures", opt);
    setOpen(false);
  };
...
export default MatchingFeaturesReading;
