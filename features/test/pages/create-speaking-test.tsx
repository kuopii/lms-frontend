...
import {
  ...
  CreateSpeakingTestSchema,
} from "../form/create-speaking-form";
...
const PassageSection = dynamic(
  () => import("../speaking/passage-section").then((mod) => mod.PassageSection),
  {
    ssr: false,
  },
);

const CreateSpeakingTestPage = () => {
  const { setTrigger, setTitle } = useFormStore();

  const form = useForm<CreateSpeakingTestSchema>({
    resolver: zodResolver(createSpeakingTestSchema),
    defaultValues: {
      title: "",
      description: "",
      difficulty: "beginner",
      ...
      passages: [
        {
          ...
          image_context: null,
          questions: [
            { question_number: 1, question_text: "", voice_limit: 0 },
          ],
        },
      ],
    },
  ...
  const onSubmit = useCallback((values: CreateSpeakingTestSchema) => {
  ...
  useEffect(() => {
    setTitle("Create a New Speaking Test");
  }, [setTitle]);
...
export default CreateSpeakingTestPage;
